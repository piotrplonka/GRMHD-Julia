...
function function_PtoU(x::AbstractVector, buffer::AbstractVector,gcov::Matrix{Float64})
    #Parameters
   ...
   #u3::Float64=x[5]
    ...
    value::Float64 = ρ + (5/3)*u + bsq # ρ + u + p + bsq = ρ + u + (γ-1)*u + bsq = ρ + γ*u + bsq ; p=(γ-1)*u, γ=5/3
    value2::Float64 = (2/3)*u + (1/2)*bsq # p + (1/2)*bsq = (γ-1)*u + bsq
...
function function_PtoFx(x::AbstractVector, buffer::AbstractVector,gcov::Matrix{Float64})
    #Parameters
    ρ::Float64  = x[1] #Density
    u::Float64  = x[2] #Internal Energy 
    u1::Float64 = x[3] #Contravariant Four-velocity in 1-direction
...
function function_PtoFy(x::AbstractVector, buffer::AbstractVector,gcov::Matrix{Float64})
    #Parameters
    ρ::Float64  = x[1] #Density
    u::Float64  = x[2] #Internal Energy 
    u1::Float64 = x[3] #Contravariant Four-velocity in 1-direction
...
function function_PtoFz(x::AbstractVector, buffer::AbstractVector,gcov::Matrix{Float64})
    #Parameters
    ρ::Float64  = x[1] #Density
    u::Float64  = x[2] #Internal Energy 
    u1::Float64 = x[3] #Contravariant Four-velocity in 1-direction
...
function Jacobian(x::AbstractVector,buffer::AbstractVector,gcov::Matrix{Float64})
	#Parameters
	ρ::Float64  = x[1] #Density
	u::Float64  = x[2] #Internal Energy 
	u1::Float64 = x[3] #Contravariant Four-velocity in 1-direction
	...
	#For u_0 u0*gcov[1,1] + u1*gcov[2,1] + u2*gcov[3,1] + u3*gcov[4,1]
	u_0_x1::Float64=0
	u_0_x2::Float64=0	
	u_0_x3::Float64=u0_x3*gcov[1,1] + gcov[2,1]
	u_0_x4::Float64=gcov[3,1]
	u_0_x5::Float64=gcov[4,1]	

	#For u_1 u0*gcov[1,2] + u1*gcov[2,2] + u2*gcov[3,2] + u3*gcov[4,2]
	u_1_x1::Float64=0
	u_1_x2::Float64=0	
	u_1_x3::Float64=u0_x3*gcov[1,2] + gcov[2,2]
	u_1_x4::Float64=gcov[3,2]
	u_1_x5::Float64=gcov[4,2]	

	#For u_2 u0*gcov[1,3] + u1*gcov[2,3] + u2*gcov[3,3] + u3*gcov[4,3]
	u_2_x1::Float64=0
	u_2_x2::Float64=0	
	u_2_x3::Float64=u0_x3*gcov[1,3] + gcov[2,3]
	u_2_x4::Float64=gcov[3,3]
	u_2_x5::Float64=gcov[4,3]

	#For u_2 u0*gcov[1,4] + u1*gcov[2,4] + u2*gcov[3,4] + u3*gcov[4,4]
	u_3_x1::Float64=0
	u_3_x2::Float64=0	
	u_3_x3::Float64=u0_x3*gcov[1,4] + gcov[2,4]
	u_3_x4::Float64=gcov[3,4]
	u_3_x5::Float64=gcov[4,4]
	...
	b1_x1::Float64=0
	b1_x2::Float64=0
	b1_x3::Float64=((b0_x3*u1+u1_x3)*u0 -(B1+b0*u1)*u0_x3 )/(u0^2)
	b1_x4::Float64=((b0_x4*u1+u1_x4)*u0 -(B1+b0*u1)*u0_x4 )/(u0^2)	
	b1_x5::Float64=((b0_x5*u1+u1_x5)*u0 -(B1+b0*u1)*u0_x5 )/(u0^2)	

	#For b2 		
	b2_x1::Float64=0
	b2_x2::Float64=0
	b2_x3::Float64=((b0_x3*u2+u2_x3)*u0 -(B2+b0*u2)*u0_x3 )/(u0^2)
	b2_x4::Float64=((b0_x4*u2+u2_x4)*u0 -(B2+b0*u2)*u0_x4 )/(u0^2)	
	b2_x5::Float64=((b0_x5*u2+u2_x5)*u0 -(B2+b0*u2)*u0_x5 )/(u0^2)		

	#For b3 		
	b3_x1::Float64=0
	b3_x2::Float64=0
	b3_x3::Float64=((b0_x3*u3+u3_x3)*u0 -(B3+b0*u3)*u0_x3 )/(u0^2)
	b3_x4::Float64=((b0_x4*u3+u3_x4)*u0 -(B3+b0*u3)*u0_x4 )/(u0^2)	
	b3_x5::Float64=((b0_x5*u3+u3_x5)*u0 -(B3+b0*u3)*u0_x5 )/(u0^2)	
