...
function HLL()
    ...
    dy = (N1_grid[end] - N1_grid[1]) / N2
    dz = (N1_grid[end] - N1_grid[1]) / N3
    c = 1.0
    tot = 0.0
    dt = 0.3 * dx / c
    ...
    while tot < T
            ...
            for j in 3:(N2-2)
                for k in 3:(N3-2)
                    @inbounds begin
                        ...
                        q_i_plus_1_2_N1 = zeros(8)
                        q_i_sub_1_2_N1  = zeros(8)
                        q_i_plus_1_2_N2 = zeros(8)
                        q_i_sub_1_2_N2  = zeros(8)
                        q_i_plus_1_2_N3 = zeros(8)
                        q_i_sub_1_2_N3  = zeros(8)

		        q_i_plus_1_2_N1, q_i_sub_1_2_N1 = WENOZ_vec(buffer_P[i-2, j, k, :],
		                                                    buffer_P[i-1, j, k, :],
		                                                    buffer_P[i,   j, k, :],
		                                                    buffer_P[i+1, j, k, :],
		                                                    buffer_P[i+2, j, k, :])
		        q_i_plus_1_2_N2, q_i_sub_1_2_N2 = WENOZ_vec(buffer_P[i, j-2, k, :],
		                                                    buffer_P[i, j-1, k, :],
		                                                    buffer_P[i, j,   k, :],
		                                                    buffer_P[i, j+1, k, :],
		                                                    buffer_P[i, j+2, k, :])
		        q_i_plus_1_2_N3, q_i_sub_1_2_N3 = WENOZ_vec(buffer_P[i, j, k-2, :],
		                                                    buffer_P[i, j, k-1, :],
		                                                    buffer_P[i, j, k,   :],
		                                                    buffer_P[i, j, k+1, :],
		                                                    buffer_P[i, j, k+2, :])
                        ...
                        PtoFx(q_i_plus_1_2_N1, local_Fx_R,
                              Kerr_Schild_metric_cov((N1_grid[i] + N1_grid[i+1])/2, N2_grid[j], N3_grid[k], a), eos)
                        PtoFx(q_i_sub_1_2_N1, local_Fx_L,
                              Kerr_Schild_metric_cov((N1_grid[i] + N1_grid[i-1])/2, N2_grid[j], N3_grid[k], a), eos)


                        PtoFy(q_i_plus_1_2_N2, local_Fy_R,
                              Kerr_Schild_metric_cov(N1_grid[i], (N2_grid[j] + N2_grid[j+1])/2, N3_grid[k], a), eos)
                        PtoFy(q_i_sub_1_2_N2, local_Fy_L,
                              Kerr_Schild_metric_cov(N1_grid[i], (N2_grid[j] + N2_grid[j-1])/2, N3_grid[k], a), eos)


                        PtoFz(q_i_plus_1_2_N3, local_Fz_R,
                              Kerr_Schild_metric_cov(N1_grid[i], N2_grid[j], (N3_grid[k] + N3_grid[k+1])/2, a), eos)
                        PtoFz(q_i_sub_1_2_N3, local_Fz_L,
                              Kerr_Schild_metric_cov(N1_grid[i], N2_grid[j], (N3_grid[k] + N3_grid[k-1])/2, a), eos)
                        ...
                        elseif C_min_N1 < 0
                            ...
                            buffer_flux[i, j, k, 1:5, 1] .= (C_min_N1 .* local_Fx_R[1:5] +
                                                             ...
                                                             C_max_N1 * C_min_N1 .* (UR_N1[1:5] .- UL_N1[1:5])) / (C_max_N1 + C_min_N1)
                        end
                        ...
                        elseif C_min_N2 < 0
                            ...
                            buffer_flux[i, j, k, 1:5, 2] .= (C_min_N2 .* local_Fy_R[1:5] +
                                                             ...
                                                             C_max_N2 * C_min_N2 .* (UR_N2[1:5] .- UL_N2[1:5])) / (C_max_N2 + C_min_N2)
                        end
                        ...
                        elseif C_min_N3 < 0
                            ...
                            buffer_flux[i, j, k, 1:5, 3] .= (C_min_N3 .* local_Fz_R[1:5] +
                                                             ...
                                                             C_max_N3 * C_min_N3 .* (UR_N3[1:5] .- UL_N3[1:5])) / (C_max_N3 + C_min_N3)
                        end


                        buffer_flux_magnetic[i, j, k, 1] = PtoF_Bx(q_i_plus_1_2_N1,
                                                                     Kerr_Schild_metric_cov((N1_grid[i+1] + N1_grid[i]) / 2, N2_grid[j], N3_grid[k], a),
                                                                     eos::Polytrope) -
                                                          PtoF_Bx(q_i_sub_1_2_N1,
                                                                   Kerr_Schild_metric_cov((N1_grid[i] + N1_grid[i-1]) / 2, N2_grid[j], N3_grid[k], a),
                                                                   eos::Polytrope)

                        buffer_flux_magnetic[i, j, k, 2] = PtoF_By(q_i_plus_1_2_N2,
                                                                     Kerr_Schild_metric_cov(N1_grid[i], (N2_grid[j+1] + N2_grid[j]) / 2, N3_grid[k], a),
                                                                     eos::Polytrope) -
                                                          PtoF_By(q_i_sub_1_2_N2,
                                                                   Kerr_Schild_metric_cov(N1_grid[i], (N2_grid[j] + N2_grid[j-1]) / 2, N3_grid[k], a),
                                                                   eos::Polytrope)

                        buffer_flux_magnetic[i, j, k, 3] = PtoF_Bz(q_i_plus_1_2_N3,
                                                                     Kerr_Schild_metric_cov(N1_grid[i], N2_grid[j], (N3_grid[k+1] + N3_grid[k]) / 2, a),
                                                                     eos::Polytrope) -
                                                          PtoF_Bz(q_i_sub_1_2_N3,
                                                                   Kerr_Schild_metric_cov(N1_grid[i], N2_grid[j], (N3_grid[k] + N3_grid[k-1]) / 2, a),
                                                                   eos::Polytrope)
                    end
                end
            end
        ...
        @threads for i in 3:(N1-2)
            for j in 3:(N2-2)
                for k in 3:(N3-2)
                    @inbounds begin
                        buffer_U[i, j, k, 1:5] .-= (dt/dx) * buffer_flux[i, j, k, 1:5, 1] .+
                                                     (dt/dy) * buffer_flux[i, j, k, 1:5, 2] .+
                                                     (dt/dz) * buffer_flux[i, j, k, 1:5, 3]
                ...
                for k in 1:N3
                    @inbounds begin
                        ...
                        if buffer_P[i, j, k, 1] < 1e-5
                            println("density < 1e-5 na ($i,$j,$k): ", buffer_P[i, j, k, 1])
                            buffer_P[i, j, k, 1] = 1e-5
                        end
                        if buffer_P[i, j, k, 2] < 1e-5
                            println("pressure < 1e-5 na ($i,$j,$k): ", buffer_P[i, j, k, 2])
                            buffer_P[i, j, k, 2] = 1e-5
                        end
                    end
                end
